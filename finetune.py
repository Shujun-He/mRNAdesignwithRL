...
for ep in range(args.episodes):
    ...
    #     print("Testing with policy only")
    #
    #     mean_aps=[]
    #     optimized_testing_sequences=[]
    #     for zipped in tqdm(testing_sequences):
    #         current_rna_sequence=zipped['sequence']
    #         protein_mask=zipped['protein_mask']
    #         for step in range(episode_length):
    #             next_rna_sequence=agent.select_action(current_rna_sequence,protein_mask,start,end,eval=True)
    #             current_rna_sequence=next_rna_sequence
    #         optimized_testing_sequences.append(current_rna_sequence)
    #         #ap=env.step(next_rna_sequence)
    #         # _,bpp=get_bpp([sequence,len(sequence),'tmp/tmp.txt'])
    #         # mean_aps.append(bpp.sum(1).mean())
    #
    #
    #     results=get_bpp_multiprocess(optimized_testing_sequences,MAX_THRE)
    #     results=np.stack([r[1] for r in results])
    #     #exit()
    #     metric=results.sum(1).mean()
    #
    #
    #
    #     if metric > best_metric:
